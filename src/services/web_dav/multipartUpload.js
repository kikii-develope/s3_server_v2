...
import { client, getBaseUrl, getRootPath, ensureDirectory, getDirectoryContents } from './webdavClient.js';
...
const uploadFileInChunks = async (dirPath, file, filename, onProgress) => {
  ...
  // 로컬 임시 디렉토리 생성
  const localTempDir = path.join(require('os').tmpdir(), `merge-${Date.now()}`);
  ...
  try {
    ...
    for (let i = 0; i < totalChunks; i++) {
      const chunkData = await fs.promises.readFile(chunkFiles[i]);
    ...
    // 로컬 임시 파일 정리
    try {
      const files = await fs.promises.readdir(localTempDir);
      for (const file of files) {
        await fs.promises.unlink(path.join(localTempDir, file));
      }
...
const mergeChunks = async (tempDir, targetDir, filename, totalChunks) => {
  ...
  const mergedPath = path.join(
    fs.mkdtempSync(path.join(require('os').tmpdir(), 'merge-')),
    filename
  );
  ...
  try {
    // 청크를 순서대로 다운로드하여 병합
    for (let i = 0; i < totalChunks; i++) {
      const chunkPath = `/${getRootPath()}/${tempDir}/chunk_${String(i).padStart(5, '0')}`;
      const chunkBuffer = await client.getFileContents(chunkPath);
    ...
    // 병합된 파일 업로드
    const finalPath = `/${getRootPath()}/${targetDir}/${filename}`;
    const fileStream = fs.createReadStream(mergedPath);
    await client.putFileContents(finalPath, fileStream);
    ...
    try {
      ...
      await fs.promises.rmdir(path.dirname(mergedPath));
    } catch (err) {

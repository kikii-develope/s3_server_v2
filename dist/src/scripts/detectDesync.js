...
const getActiveFiles = async (limit = 1000, offset = 0) => {
    const [rows] = await database_js_1.default.execute(`SELECT id, file_path, file_name, file_size, content_hash, etag
         FROM file_metadata
         WHERE status = 'ACTIVE'
         ORDER BY id
...
const getTotalActiveCount = async () => {
    const [rows] = await database_js_1.default.execute(`SELECT COUNT(*) as total FROM file_metadata WHERE status = 'ACTIVE'`);
    return rows[0].total;
};
/**
 * WebDAV에서 파일 정보 조회
...
const getFileHash = async (filePath) => {
    try {
        ...
        return (0, etag_js_1.calculateHash)(Buffer.from(content));
    }
    catch (error) {
...
const verifyFile = async (file) => {
    ...
    try {
        ...
        if (file.file_size && fileInfo.size !== file.file_size) {
            stats.sizeMismatch++;
            await markAsDesync(file.id, file.file_path, `파일 크기 불일치 (DB: ${file.file_size}, NAS: ${fileInfo.size})`);
            return;
        }
        ...
        if (verifyHash && file.content_hash) {
            ...
            if (actualHash && actualHash !== file.content_hash) {
                stats.hashMismatch++;
                await markAsDesync(file.id, file.file_path, `해시 불일치 (DB: ${file.content_hash.substring(0, 16)}..., NAS: ${actualHash.substring(0, 16)}...)`);
                return;
            }
        }
        stats.ok++;

 ...
 * @param {Object} res - Express response 객체
 */
const downloadFileFromWebDAV = async (req, res) => {
    try {
...
exports.downloadFileFromWebDAV = downloadFileFromWebDAV;
/**
 * WebDAV 디렉토리 생성 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const createWebDAVDirectory = async (req, res) => {
    try {
        const { path } = req.body;
        if (!path) {
            return (0, response_js_1.errorResponse)(res, 'path가 필요합니다.', 400);
        }
        await (0, webdavClient_js_1.createDirectory)(path);
        return (0, response_js_1.successResponse)(res, 'WebDAV 디렉토리 생성 성공', { path });
    }
    catch (error) {
...
exports.createWebDAVDirectory = createWebDAVDirectory;
/**
 * WebDAV 디렉토리 목록 조회 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const getWebDAVDirectory = async (req, res) => {
    try {
...
exports.getWebDAVDirectory = getWebDAVDirectory;
/**
 * WebDAV 서버 정보 조회 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const getWebDAVInfo = async (req, res) => {
    try {
        const baseUrl = (0, webdavClient_js_1.getBaseUrl)();
        return (0, response_js_1.successResponse)(res, 'WebDAV 서버 정보 조회 성공', {
            baseUrl,
...
exports.uploadMultipleFilesToWebDAV = uploadMultipleFilesToWebDAV;
/**
 * WebDAV 파일 업데이트 (덮어쓰기) 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const updateFileInWebDAV = async (req, res) => {
    try {
        const rawPath = req.params[0] || req.params.path;
        const file = req.file;
        ...
        const filePath = extractFilePath(rawPath);
        if (!userId) {
            return (0, response_js_1.errorResponse)(res, 'userId가 필요합니다.', 400);
        }
        // 경로에서 디렉토리와 파일명 분리
        ...
        if (!originalExtension) {
            ...
            const contents = await (0, webdavClient_js_1.getDirectoryContents)(searchPath);
            if (contents) {
                let matchedFile = contents.find(item => item.type === 'file' &&
                    item.basename.includes('.') &&
                    item.basename.split('.').slice(0, -1).join('.').normalize('NFKC') === filename.normalize('NFKC'));
                if (!matchedFile) {
                    matchedFile = contents.find(item => item.type === 'file' &&
                ...
                if (matchedFile) {
                    ...
                    return (0, response_js_1.errorResponse)(res, `파일을 찾을 수 없습니다: ${filename}`, 404);
                }
            }
            else {
                return (0, response_js_1.errorResponse)(res, `디렉토리를 찾을 수 없습니다: ${directoryPath}`, 404);
            }
        }
        // 실제 파일 경로 (확장자 포함)
        const actualFilePath = directoryPath ? `${directoryPath}/${filename}` : filename;
        // MIME 타입 검증
        const originalMime = originalExtension ? mime_types_1.default.lookup(originalExtension) : null;
        const uploadMime = file.mimetype;
        if (originalMime && originalMime !== uploadMime) {
            return (0, response_js_1.errorResponse)(res, `파일 타입이 다릅니다. 기존: ${originalMime}, 업로드: ${uploadMime}. 삭제 후 새로 업로드해주세요.`, 409);
        }
        // file_metadata 조회
        let metadata = await fileMetadataRepo.findByFilePath(actualFilePath);
        // 기존 파일 내용 조회 (ETag lazy 생성용)
        const fullPath = `${(0, webdavClient_js_1.getBaseUrl)()}/${actualFilePath}`;
        const existingFileBuffer = await (0, webdavClient_js_1.getFile)(fullPath);
        if (!existingFileBuffer) {
            return (0, response_js_1.errorResponse)(res, '파일을 찾을 수 없습니다.', 404);
        }
        // metadata가 없으면 lazy 생성 후 428 반환
        if (!metadata) {
            const contentHash = (0, etag_js_1.calculateHash)(existingFileBuffer);
            const currentEtag = (0, etag_js_1.generateEtag)(contentHash);
            const mimeType = mime_types_1.default.lookup(originalExtension) || 'application/octet-stream';
            metadata = await fileMetadataRepo.create({
            ...
            res.set('ETag', (0, etag_js_1.formatEtagHeader)(currentEtag));
            return (0, response_js_1.errorResponse)(res, 'If-Match 헤더가 필요합니다. ETag를 확인 후 재요청해주세요.', 428, {
                etag: currentEtag
            });
        }
        // ETag가 없으면 lazy 생성 후 428 반환
        if (!metadata.etag) {
            const contentHash = metadata.content_hash || (0, etag_js_1.calculateHash)(existingFileBuffer);
            const currentEtag = (0, etag_js_1.generateEtag)(contentHash);
        ...
        // If-Match 헤더 체크
        const ifMatch = (0, etag_js_1.parseIfMatchHeader)(req.headers['if-match']);
        if (!ifMatch) {
            res.set('ETag', (0, etag_js_1.formatEtagHeader)(metadata.etag));
            return (0, response_js_1.errorResponse)(res, 'If-Match 헤더가 필요합니다.', 428, {
                etag: metadata.etag
            });
        }
        // ETag 비교
        if (!(0, etag_js_1.compareHash)(ifMatch, metadata.etag)) {
            res.set('ETag', (0, etag_js_1.formatEtagHeader)(metadata.etag));
            return (0, response_js_1.errorResponse)(res, '파일이 변경되었습니다. 최신 버전을 다시 받아주세요.', 412, {
                etag: metadata.etag
            });
        }
        // 새 파일 해시 계산 (스트림 방식)
        ...
        res.set('ETag', (0, etag_js_1.formatEtagHeader)(newEtag));
        return (0, response_js_1.successResponse)(res, '파일 업데이트 성공', {
            path: actualFilePath,
...
exports.updateFileInWebDAV = updateFileInWebDAV;
/**
 * WebDAV 파일 삭제 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const deleteFileFromWebDAV = async (req, res) => {
    try {
        const rawPath = req.params[0] || req.params.path;
        ...
        const filePath = extractFilePath(rawPath);
        // 실제 파일 삭제
        await (0, webdavClient_js_1.deleteFile)(filePath);
        ...
        return (0, response_js_1.successResponse)(res, '파일 삭제 성공', { path: filePath });
    }
    catch (error) {
...
exports.deleteFileFromWebDAV = deleteFileFromWebDAV;
/**
 * WebDAV 디렉토리 삭제 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const deleteDirectoryFromWebDAV = async (req, res) => {
    try {
        const rawPath = req.params[0] || req.params.path;
        ...
        await (0, webdavClient_js_1.deleteDirectory)(dirPath);
        return (0, response_js_1.successResponse)(res, '디렉토리 삭제 성공', { path: dirPath });
    }
    catch (error) {
        console.error('WebDAV 디렉토리 삭제 에러:', error);
        if (error.status === 404 || error.message?.includes('not found')) {
            return (0, response_js_1.errorResponse)(res, '디렉토리를 찾을 수 없습니다.', 404);
        }
        return (0, response_js_1.errorResponse)(res, error.message);
    }
};
exports.deleteDirectoryFromWebDAV = deleteDirectoryFromWebDAV;
/**
 * WebDAV 파일/디렉토리 이동 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const moveFileInWebDAV = async (req, res) => {
    try {
        const { sourcePath: rawSourcePath, destPath: rawDestPath, overwrite = true } = req.body;
        if (!rawSourcePath) {
            return (0, response_js_1.errorResponse)(res, 'sourcePath가 필요합니다.', 400);
        }
        if (!rawDestPath) {
            return (0, response_js_1.errorResponse)(res, 'destPath가 필요합니다.', 400);
        }
        // URL에서 실제 경로 추출
        const sourcePath = extractFilePath(rawSourcePath);
        const destPath = extractFilePath(rawDestPath);
        await (0, webdavClient_js_1.moveFile)(sourcePath, destPath, overwrite);
        return (0, response_js_1.successResponse)(res, '이동 성공', { sourcePath, destPath });
    }
    catch (error) {
        console.error('WebDAV 이동 에러:', error);
        if (error.status === 404 || error.message?.includes('not found')) {
            return (0, response_js_1.errorResponse)(res, '원본 파일/디렉토리를 찾을 수 없습니다.', 404);
        }
        if (error.status === 412 || error.message?.includes('precondition')) {
            return (0, response_js_1.errorResponse)(res, '대상이 이미 존재합니다.', 409);
        }
        return (0, response_js_1.errorResponse)(res, error.message);
    }
};
exports.moveFileInWebDAV = moveFileInWebDAV;
/**
 * WebDAV 파일/디렉토리 복사 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const copyFileInWebDAV = async (req, res) => {
    try {
        const { sourcePath: rawSourcePath, destPath: rawDestPath, overwrite = true } = req.body;
        if (!rawSourcePath) {
            return (0, response_js_1.errorResponse)(res, 'sourcePath가 필요합니다.', 400);
        ...
        const sourcePath = extractFilePath(rawSourcePath);
        const destPath = extractFilePath(rawDestPath);
        await (0, webdavClient_js_1.copyFile)(sourcePath, destPath, overwrite);
        return (0, response_js_1.successResponse)(res, '복사 성공', { sourcePath, destPath });
    }
    catch (error) {
...
exports.copyFileInWebDAV = copyFileInWebDAV;
/**
 * 시스템 통계 조회 컨트롤러
 * @param {Object} req - Express request 객체
 * @param {Object} res - Express response 객체
 */
const getWebDAVStats = async (req, res) => {
    try {
        // file_metadata 요약
        const [summaryRows] = await database_js_1.default.execute(`
            SELECT
                COUNT(*) as totalFiles,
        ...
        `);
        // history 액션별 통계
        const [historyRows] = await database_js_1.default.execute(`
            SELECT action, COUNT(*) as count
            FROM file_metadata_history
            GROUP BY action
        `);
        // 사용자별 통계
        const [userRows] = await database_js_1.default.execute(`
        ...
        `);
        // 최근 7일 일별 통계
        const [dailyRows] = await database_js_1.default.execute(`
            SELECT
        ...
        return (0, response_js_1.successResponse)(res, '통계 조회 성공', {

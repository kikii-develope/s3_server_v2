...
const getUniqueFilename = async (dirPath, filename) => {
    ...
    const contents = await (0, exports.getDirectoryContents)(`/www${normalizedPath}`);
    if (!contents) {
        return filename;
    }
...
const uploadSingle = async (path, file, filename) => {
    try {
        ...
        return {
            ...
            success: true,
            size: f.size,
            url: (0, exports.getBaseUrl)() + `/www/${path}/${f.originalname}`,
...
const ensureDirectory = async (path) => {
    ...
    for (const part of parts) {
        ...
        // 1) 이미 있으면 통과
        const exists = await (0, exports.existDirectory)(`/www${next.startsWith('/') ? '' : '/'}${next}`);
        if (!exists) {
            try {
